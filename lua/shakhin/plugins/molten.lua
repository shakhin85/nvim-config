return {
  -- Jupyter-like интерактивная разработка в Neovim
  {
    ...
    dependencies = {
      "3rd/image.nvim", -- для отображения изображений
    },
    build = ":UpdateRemotePlugins",
    init = function()
      -- Основные настройки
      vim.g.molten_image_provider = "image.nvim" -- используем image.nvim для картинок
  ...
  -- Image.nvim для отображения картинок
  {
    "3rd/image.nvim",
    cond = function()
      -- Не загружаем в headless режиме
      return not vim.g.headless
    end,
    opts = {
      backend = "kitty", -- или "ueberzug" для других терминалов
      integrations = {
        markdown = {
          enabled = true,
